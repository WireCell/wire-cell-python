...
def load_detector_config(detector, **overrides):
    ...
    for k, v in overrides.items():
        if v is not None:
            cfg[k] = v
    for key in ('gain', 'shaping', 'adc_tick', 'output_window'):
...
def make_track_response(fr, cfg):
    ...
    # Optionally extend the working window to avoid circular-convolution
    # wraparound when the FR file's native length is shorter than the
    # FR⊗ER response duration (e.g. PDVD bipolar induction tail).
    output_window = cfg.get('output_window')
    if output_window is not None and output_window > N_fr_native * period:
        N_fr = int(round(output_window / period))
    else:
        N_fr = N_fr_native
...
def export_chndb_resp(waves, cfg, path, detector=None):
    """Write a chndb-resp.jsonnet file from the given FR⊗ER ADC waveforms.

    waves    - dict {'U': ndarray, 'V': ndarray} as returned by make_track_response().
    cfg      - detector config dict (gain, shaping, postgain, adc_per_mv, adc_tick, fr).
    path     - output file path.
    detector - detector key string (e.g. 'pdhd', 'pdvd-bottom'); used in header comment.

    The stored arrays reflect the reference gain baked into cfg.
    When using these arrays in chndb-base.jsonnet, multiply each element by
    gain_scale = params.elec.gain / (ref_gain) so the kernel tracks the runtime FE gain.
    """
    tick_ns = cfg['adc_tick'] / units.ns
    er_kind = cfg.get('er_kind', 'cold')
    if er_kind == 'cold':
        gain_str  = f"{cfg['gain'] / (units.mV / units.fC):.1f} mV/fC"
        shaping_str = f"{cfg['shaping'] / units.us:.1f} us"
        gain_ref = f"{cfg['gain'] / (units.mV / units.fC):.1f} * wc.mV / wc.fC"
        scale_note = (
            "// These arrays are stored at the reference gain above.  In chndb-base.jsonnet\n"
            "// apply:  std.map(function(x) x * gain_scale, u_resp)\n"
            f"// where   gain_scale = params.elec.gain / ({gain_ref})\n"
            "// so the kernel scales with runtime FE gain.\n"
        )
    else:
        gain_str  = f"(json ER: {cfg.get('er_file', '?')})"
        shaping_str = 'n/a'
        scale_note = (
    ...
    det_label = detector if detector else 'unknown'
    header = (
        f"// {det_label.upper()} FR⊗ER coherent-noise deconvolution kernel.\n"
        "//\n"
        "// Generated by:\n"
        f"//   wirecell-sigproc track-response -d {det_label} --export-jsonnet <path>\n"
        "//\n"
        f"// FR file   : {cfg['fr']}\n"
        f"// gain      : {gain_str}\n"
        f"// shaping   : {shaping_str}\n"
        f"// postgain  : {cfg['postgain']}\n"
        f"// ADC/mV    : {cfg['adc_per_mv']}\n"
        f"// tick      : {tick_ns:.0f} ns  (post-resampler; NF chain runs at 500 ns)\n"
        "//\n"
        + scale_note +
        "//\n"
        "// response_offset (argmin of each array):\n"
    )
    ...
    for label in ('U', 'V'):
        ...
        key = label.lower() + '_resp'
        lines.append(f"//   {key}: argmin = {argmin}\n")
    lines.append("//\n{\n")
