...
def export_chndb_resp(waves, cfg, path):
    ...
    The stored arrays reflect the reference gain baked into cfg (typically 14 mV/fC).
    When using these arrays in chndb-base.jsonnet, multiply each element by
    gain_scale = params.elec.gain / (14*wc.mV/wc.fC) so the kernel tracks the
    runtime FE gain.
    """
    ...
    header = (
        "// PDHD FR⊗ER coherent-noise deconvolution kernel.\n"
        "//\n"
        "// Generated by:\n"
        "//   wirecell-sigproc track-response -d pdhd --export-jsonnet <path>\n"
        "//\n"
        f"// FR file   : {cfg['fr']}\n"
        f"// gain      : {gain_str}\n"
        ...
        "// where   gain_scale = params.elec.gain / (14.0 * wc.mV / wc.fC)\n"
