...
def magnify_dump(ctx, out, filename):
    ...
    t2 = time()
    log.debug (t2-t1) # takes 5.7 seconds compared to 5.3 seconds loading npz
    log.debug ("Writing NPZ file")
...
def npz_load(ctx, out, filename):
    ...
    t2 = time()
    log.info (f'Memmap load: {t2-t1}')
    arrs2 = numpy.load(filename,None)
    t3 = time()
    log.info (f'Full load: {t3-t2}')
    outs=dict()
    for name in arrs1:
        arr1 = arrs1[name]
        arr2 = arrs2[name]
        ...
        outs[name] = arr
    t4 = time()
    log.info (f'Subtract: {t4-t3}')
    numpy.savez_compressed(out, **outs)
    t5 = time()
    log.info (f'Done: {t5-t4}')
